...
class PriceServiceTest {
	...
	void testObtenerListaPrecios() {
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd-HH.mm.ss");
        LocalDateTime format = LocalDateTime.parse("2020-06-14-16.00.00", formatter);
    	price.setFechaAplicacion(Timestamp.valueOf(format));
    	
		Mockito.when(priceDao.obtenerListaPrecios(Mockito.any(Price.class))).thenReturn(list);
		Price resultado = priceService.obtenerListaPrecios(price);
		assertEquals(resultado, price);
	}
